...
def getMetrics():
    if request.method == "POST":
        try:
            ...
            grid = GridSearchCV(estimator=clf_rf, param_grid=param_grid, cv = 3, n_jobs=-1)
            grid_result = grid.fit(X_train, y_train)
            clf_rf = RandomForestClassifier(**grid_result.best_params_)
            ...
            param_grid = dict(criterion=criterion,max_features=max_features,n_estimators=n_estimators,max_depth=max_depth)
            grid_result = grid.fit(X_train, y_train)
            clf_et = ExtraTreesClassifier(**grid_result.best_params_)
            ...
            elif metrics_et[2] > metrics_xgb[2]:
